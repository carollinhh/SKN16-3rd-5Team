...
| 진세현 |모델이 제시한 높은 점수와 실제 결과물의 품질이 항상 일치하지는 않는다는 점을 배웠습니다. 그래서 저희는 수치에만 의존하기보다, 팀원들과 함께 직접 결과물을 비교하며 가장 설득력 있는 결과물을 선택했습니다. 이번 경험으로 좋은 모델을 만들기 위해서는 객관적인 데이터와 팀의 종합적인 판단을 결합하는 것이 핵심임을 명확히 깨달았습니다. |
| 김나은 | RAG 기반 챗봇 프로젝트를 수행하는 과정에서 미흡한 부분도 있었으나, 출중한 팀원들의 전문성과 협력 덕분에 문제를 해결하며 많은 것을 배우고 성장할 수 있었다.|
